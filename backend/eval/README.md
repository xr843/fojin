...
### 等价来源（`relevance: 1`）补录规则（v1.4 起）

v1.3 的宽松 miss 里，相当一部分是检索找到了站得住的出处、只是 gold 没收（如慧能生平→《指月錄》、
四念处→《雜阿含經》）。v1.4 读了召回段落的**正文**后补录 43 条，全部 relevance=1，规则如下：

- **补**：① 经、律、论本典中直接讲该主题的（印度论师的释论收在论藏，算「论」，如《大智度論》《攝大乘論釋》）；
  ② 专门论著（独立成书，不是随文作注，如《法界次第初門》《北山錄》）；
  ③ 史实题的僧传、灯录、编年史（如《五燈會元》《佛祖統紀》）。
- **不补**：汉地的随文注疏（疏、鈔、記、註、解、玄贊、科註……），即使那一段讲得很对；冷僻晚出的汇编也不补。
  注疏偏置是检索器的已知病（正藏先验 `ENABLE_CANONICAL_PRIOR` 就是治它的），把注疏收成等价来源，
  宽松指标就既看不见病、也看不见疗效。
- **判法**：必须读召回段落的正文，不能只看经名。
- **只加 relevance=1**：v1.4 的严格口径与 v1.3 逐题相同，严格数字可以跨版本直接对照。

> ⚠️ **这把尺子偏向现状，评估检索改动时必须对称裁决。** 这 37 条是从「当前检索器漏判的题」里挖出来的：
> 在发现批次上它们必然命中（宽松 Hit@5 0.466→0.753 是补录构造出来的，不是检索变好了）。
> 更要紧的是，现状召回到的合法替代源有分，别的检索方案召回到的另一些合法来源却没分。
> 所以评估任何检索改动时，要把**实验臂新出现、又不在 gold 里的经名**按上面的规则裁决一遍，
> 合法的补进 gold 后两臂重算。只看宽松数字的升降，会系统性地偏袒现状。
>
> v1.4 本身就是这样做的两轮：第一轮从现状（正藏先验关）的 39 道宽松 miss 题里补了 37 条；
> 第二轮从正藏先验**开启**臂新冒出来的经名里，用同一规则补了 6 条（如结集题的《根本說一切有部毘奈耶雜事》）。
> 只做第一轮时，先验在宽松口径上显示为 −2.7pp；两轮补齐后才是公平的对照。
...
## 找回丢失的出处（`recover_citations.py`）——离线，判断该不该上线

问题：模型有时凭参数记忆引出真经文，而检索没召回它，于是这句服务出去时**没有任何可点开的出处**。
30 天实测：461 条无标记答案里，**229 处引文是真经文、涉及 109 篇答案（约 7.6%）**，
且按定义都不在该次召回片段里。

```bash
python -m eval.recover_citations --window-days 30 --control 120
```

### 结论（2026-09-02）：**不要整体自动补引用**

| 测量 | 值 | 含义 |
|---|---|---|
| 回收率（能定出出处） | **26.7%** | 四分之三仍然无法归属 |
| 其中只对应唯一一部经 | 49.8% | 中位命中 2 部经 |
| 精度 · **唯一解析** | **96.3%** | 唯一可上线的子集 |
| 精度 · 多经命中 | 40.0% | 不可用 |

⚠️ **池化精度读作 81%，它不描述上面任何一个总体。** 只报池化会掩盖唯一决定该不该上线的事实。
多经命中那一档的失败是**系统性**的：解析器偏向根本经，而模型标的是同样含这句话的注疏
（《地藏本願經科註》vs《地藏菩薩本願經》）。解析器往往更对，但它在推翻一个**运行时护栏已核验过**
的归属——这不是能悄悄做的交易。

**可上线的窄子集存在且事先可判定**（`unique_text`），约每月 114 处、精度 96.3%。
但这些答案目前是**完全没有引用**的，而准则明写「宁可显示未核实，也不要给一个看起来可点、
实则错位的引用」——1/27 的错误率意味着应该以「疑似出自…」的线索形式呈现，而不是当作已核验的引用。

### 方法：ES 只做召回，判定用 `normalise_for_match`

对照臂（把护栏已判逐字正确的引文遮住引用标记，看能否还原）是这个模块的地基。
三次方法都是被对照臂否掉的：

| 尝试 | 对照臂 | 判决 |
|---|---|---|
| 只找 `「」` 就算引文 | — | ⛔ 复活了 PR #952 修掉的「强调号当引用」假阳性 |
| `text_contents LIKE` | — | ⛔ 简体探针查繁体语料，且省略号必然失配 |
| ES `match_phrase` 直接判 | **30.0%** | ⛔ CJK 分词在标点处断，真经文被判缺失 |
| **ES 召回 + `normalise_for_match` 判定** | **98.8%** | ✅ 采用 |
