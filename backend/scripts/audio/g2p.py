...
def to_indextts_text(
    ...
    ⚠️ ``minimal`` **默认 False（全量标注），这是实测结论，不要想当然改成 True**。

    曾假设「密集标注会让模型插字」（「祇樹給孤獨園」6 字被念成 8 音节，那里正好
    连着 5 个标签），据此做了 ``minimal=True``：逐字比对，只标 pypinyin 会读错的字，
    标注数 12 → 4。**A/B 实测（2026-08-12，金剛經开经段 + 用户诵经参考音）直接推翻了它**：

    | 配置 | 音节匹配率 |
    |---|---|
    | 全量标注 + duration_factor 1.15 | **76%** |
    | 全量标注 | 74% |
    | 最小标注 + 1.15 | 61% |
    | 最小标注 | 61% |
    | 无标注 | 59% |

    最小标注几乎跌回无标注的水平。**原因：判断「哪些标注多余」时我拿 pypinyin
    当基准，但模型有自己的 G2P，不是 pypinyin** —— 那些对 pypinyin 而言多余的
    标注（祇樹、比丘），对模型可能一点都不多余。

    ``minimal=True`` 保留下来仅供对照实验，生产路径不要用。
