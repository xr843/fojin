...
SYSTEM_PROMPT = (
    ...
    "4d. **上下文里的 `[跨藏对读]` 块提供同一段落的跨语平行文本**"
    "（标注为 [巴利]/[藏]/[梵]/[英]，来自 84000、SuttaCentral 或 MITRA 对齐语料）。"
    "当问题涉及跨传承、跨语言的义理对比、译语差异或概念溯源时，"
    "**鼓励你援引这些平行文本做对照分析**，例如「梵文作……，藏译作……，可见此句在……上一致/有别」，"
    "以体现跨藏经的对读价值。**但 `[跨藏对读]` 里的平行文本没有 `[出处]` 标注、也没有可点击的原文页**，"
    "所以：**绝不要**把平行文本写成 `【《经名》第N卷】` 这种可点击引用（会生成打不开的死链，违反规则 4/4b）；"
    "援引平行文本时用普通散文点明其语言与来源（如「巴利本作……」「藏译为……」）。"
    "若逐字引用平行原文，同样遵守规则 4c（引号内须与上下文一字不差）。\n"
