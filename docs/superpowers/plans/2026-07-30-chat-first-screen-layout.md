...
| 12 | **有对话且消息多到出现滚动条** | 消息区的居中列与输入区的居中列**左右边缘是否对齐**。三个 `.chat-column-inner` 是独立节点，消息区那个在滚动容器内 —— 滚动条出现时该容器少约 15px 可用宽度，840 居中列会相对偏左约 7.5px。Task 2 审查已指出此风险。若肉眼可见错位，在 `.chat-column-inner` 的滚动容器上加 `scrollbar-gutter: stable`（消息滚动区那一处，不是三处都加）后复看 |
...
若第 3 步不轮换了，说明 TextArea 被 remount、`ChatPage.tsx:1058` 的原生监听掉了 —— 回到 Task 3 Step 5 检查撑高块是否被插到了输入区**之前**（那会让输入区的子节点槽位位移）。
