...
#### ✅ 定型合成参数（2026-08-12 实测，用户本人诵经参考音）

| 参数 | 值 | 依据 |
|---|---|---|
| 标注 | **全量**（`minimal=False`） | A/B 实测：全量 74~76% vs 最小 61% vs 无标注 59% |
| `duration_factor` | **1.15** | 语速 1.83 音节/秒 vs 人声 1.85，几乎分毫不差 |
| `text_normalization` | `False` | 否则繁体转简体 |
| `use_cuda_kernel` | `False` | 否则 nvcc compute_70 编译失败 |
| 分句上限 | **38 字** | 压在 IndexTTS 内部分段阈值 40 之下 |

声学比对（用户读同一段文本的原录音为基准）：

| 指标 | 人声 | 定型配置 | 其余最好的 |
|---|---:|---:|---:|
| 语速(音节/秒) | 1.85 | **1.83** | 2.16 |
| 谐噪比(dB，清晰度) | 4.22 | **3.89**（92%） | 2.97 |
| F0 中位(Hz) | 111.3 | **112.6** | 114.9 |
| F0 标准差 | 13.8 | **14.6** | 17.9 |

⚠️ **`interval_silence` 不要设为 0**。合成音里那 3 处不自然停顿确实来自它（默认 200ms，
段间插入），但设 0 会引入拼接不连续，谐噪比从 3.89 掉到 2.54，更糟。正解是把分句上限
压到 38 字，**根本不触发内部分段**——金剛經全卷 570 句实测平均 14.3 字、中位 10 字，
仅 5.1% 超过 40 字。

⚠️ **ASR 匹配率是有天花板的尺子**：用户本人清晰朗读同一段，whisper small 也只给 78%。
定型配置拿到 76%，即已接近人声。判断音质要用声学指标（尤其谐噪比）配合人耳，
不能只看 ASR。
