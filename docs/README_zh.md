...
- **AI 佛学问答**：内置「小津」(XiaoJin) AI 助手，基于 pgvector RAG 语义检索，覆盖 7,290 部经文（34.7 万语义片段）。`/chat` 专页支持流式输出、引用跳转、对话导出，回答基于经文原文并附引用出处。**在线阅读页内嵌可拖拽 AI 解读分屏**（默认打开，左右独立滚动），元问题（"你是谁 / 你能做什么"）会跳过 RAG 直接返回功能介绍
