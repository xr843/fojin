...
聚合全球佛教数字资源 —— 503 数据源中的 10,500+ 部经典（巴利、汉传、藏传、梵文四大传统），共 23,500+ 卷全文。**全球首个 LLM 驱动的三藏对读 RAG 平台**（CBETA × SuttaCentral × 84000），段落级跨藏经对应由 LLM 验证；同时提供 CBETA 风格阅读、含 14 位法师人格的 AI 问答（RAG + 宗派限定检索 + 引用）、含 31K+ 实体和 28K+ 关系的知识图谱可视化（基于 Deck.GL 的 50K 地理实体地图）、32 部辞典 748K 词条（覆盖 6 语种）、时间线可视化、动态信息流、收藏夹、引用导出、批注、书签、多语对读阅读。
...
| **跨藏经对读经文** | **三语跨藏对读** —— 3,500+ 条 LLM 验证段对，覆盖《妙法蓮華經》↔ Toh 113（**259 对，2026-06-08**）、《小品般若波羅蜜經》↔ Toh 11 Aṣṭasāhasrikā（**127 对，2026-06-09**）、维摩诘、心经、四阿含 ↔ 尼柯耶全集、法句等 11 个 pair 定义 |
...
| 学某位法师 | **法师人格模式** 14 位历史佛教大师，每位限定其宗派经典 RAG |
...
选一位佛教法师，按其教学风格回答，限定其宗派核心经典 RAG。14 位历史法师可选：
...
| 缓存 | Redis 7 |
| AI | RAG（678K+ 向量，BGE-M3，HNSW）+ 14 法师人格 + 多 LLM 厂商（OpenAI/Anthropic/DeepSeek/DashScope/Gemini/+10 家） |
...
- [x] **三语跨藏经对读** —— 11 个 pair 定义，3,500+ 条 LLM 验证对齐（CBETA / SuttaCentral / 84000；含法华 ↔ Toh 113、八千颂般若 ↔ Toh 11 两部大乘汉藏批次、四阿含 ↔ 尼柯耶全集、原 MVP 5 经）
